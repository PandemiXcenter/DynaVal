...
Status: implementation specification; the application has not been built yet.
The current deliverables are this plan, AGENTS.md, and a locked uv environment.
...
| Build automation | The user sets up GitHub Actions; the project supplies reproducible PyInstaller commands and platform requirements |
...
Planned package layout, created incrementally:
...
src/dynaval/
  ...
  storage/{paths,database,repositories,migrations,locking}.py
  services/{importing,sessions,media,exporting,reporting}.py
  ui/{setup,validation,sessions,summary}.py
  ui/components/{image_viewer,field_editor,progress}.py
tests/{unit,integration,fixtures}/
packaging/                      # spec/hooks/icons once the packaging spike passes
.github/workflows/              # build automation to be set up by the user
```
...
from rendering. Introduce an installable `src` package and uv build backend in the
first implementation milestone; no unused package entry point is declared today.
...
Preparation checks: the locked environment and imports were verified on macOS
...
After the application package/tests exist:
...
6. **Release readiness and build handoff.** Supply packaging assets/hooks, stable
   ...
   for both platforms. The user configures GitHub Actions around this contract.
...
Both operating systems are required distribution targets. The user will configure
GitHub Actions to compile/package them. This project supplies the reproducible
build interface and expected outputs; workflow YAML is not part of this setup task.

Use NiceGUI's `nicegui-pack` wrapper, which invokes PyInstaller and collects
NiceGUI data. Start with an `--onedir --windowed` bundle to simplify diagnosis and
startup. Native configuration needed by the child process belongs at import
...
After the launcher is implemented, run these commands independently on macOS and
Windows runners with Python 3.13 and uv:

```sh
uv sync --locked --all-groups
uv run --group build nicegui-pack --onedir --windowed --name DynaVal main.py
```

In the spike, derive a checked-in PyInstaller spec/hooks from the working builds
and add explicit `src` package resolution, app assets, platform-specific icons
(`.icns`/`.ico`), a macOS bundle identifier, and version metadata. Keep macOS BUNDLE
construction and Windows resources conditional on the build platform. Continue builds with
`uv run --group build pyinstaller packaging/DynaVal.spec` once that file exists.
Keep NiceGUI data collection equivalent to the verified wrapper configuration.
...
The handoff for the user's Actions workflow is: select a native runner per target,
install uv/Python 3.13, sync with `uv sync --locked --all-groups`, run the documented
Ruff/mypy/pytest checks once those tests exist, run the shared packaging command,
and archive each full bundle with its version/platform/architecture. Publish
SHA-256 checksums alongside eventual GitHub Release assets. Keep trigger choices,
workflow credentials, and publication automation with the user's Actions setup.
